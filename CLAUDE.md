...
- **shared export helpers**: `--in-chans` currently supports only `1` or `3`; for 1-channel models, shared normalization and calibration logic averages RGB mean/std values down to a single grayscale value
- **onnx**: `--slim` (default `True`) runs onnxslim after export for graph optimization (constant folding, dead-code elimination, operator fusion); disable with `--no-slim`; `--normalize` embeds timm mean/std normalization into the model; `--softmax` adds a softmax output layer (implies `--normalize`); `--mean`/`--std` override timm config for custom normalization
...
- **torchscript**: `--method` selects `trace` (default, recommended) or `script`; `--normalize` embeds timm mean/std normalization into the model; `--softmax` adds a softmax output layer (implies `--normalize`); `--mean`/`--std` override timm config for custom normalization
