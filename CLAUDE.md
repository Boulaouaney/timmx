...
- **onnx**: `--slim` (default `True`) runs onnxslim after export for graph optimization (constant folding, dead-code elimination, operator fusion); disable with `--no-slim`; `--normalize` embeds timm mean/std normalization into the model; `--softmax` adds a softmax output layer (implies `--normalize`)
...
- **torchscript**: `--method` selects `trace` (default, recommended) or `script`; `--normalize` embeds timm mean/std normalization into the model; `--softmax` adds a softmax output layer (implies `--normalize`)
