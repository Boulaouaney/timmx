...
- **coreml**: `--source` selects model capture: `trace` (default, `torch.jit.trace`) or `torch-export` (beta, `torch.export.export()`); with `torch-export`, `ct.convert()` auto-infers shapes from the `ExportedProgram` and `--dynamic-batch` requires `--batch-size >= 2`; `--batch-upper-bound` applies to both sources (sets `max=` on `torch.export.Dim` for torch-export, `ct.RangeDim.upper_bound` for trace); `--compute-precision` is only valid with `--convert-to mlprogram`; weight quantization via `--half` (fp16), `--int8` (8-bit k-means), `--int4` (4-bit k-means, mlpackage only) — mutually exclusive; neuralnetwork uses `ct.models.neural_network.quantization_utils.quantize_weights()`, mlprogram uses `ct.optimize.coreml.palettize_weights()` for int8/int4 (--half is a no-op on mlprogram since weights are already fp16); k-means requires `scikit-learn>=0.17,<=1.5.1` (included in the `coreml` extra)
